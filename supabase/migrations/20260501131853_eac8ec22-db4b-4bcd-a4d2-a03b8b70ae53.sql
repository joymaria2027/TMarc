-- Always credit the rider who actually performed the delivery
...
BEGIN
  IF NEW.settlement_approved = true AND (OLD.settlement_approved IS DISTINCT FROM true) THEN
    ...
    -- Always use the rider from the actual delivery
    ...
    -- Rider per-restaurant wallet (always the delivery's rider)
...
-- Clear legacy rider pinning so rules become restaurant-scoped
UPDATE public.revenue_sharing SET rider_id = NULL WHERE rider_id IS NOT NULL;