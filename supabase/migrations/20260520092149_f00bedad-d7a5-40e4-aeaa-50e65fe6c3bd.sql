-- 1. Rename tables
ALTER TABLE public.companies RENAME TO merchants;
ALTER TABLE public.company_riders RENAME TO merchant_riders;
ALTER TABLE public.company_tariffs RENAME TO merchant_tariffs;

-- 2. Rename columns
ALTER TABLE public.merchant_riders RENAME COLUMN company_id TO merchant_id;
ALTER TABLE public.merchant_tariffs RENAME COLUMN company_id TO merchant_id;
ALTER TABLE public.deliveries RENAME COLUMN company_id TO merchant_id;
ALTER TABLE public.expense_alerts RENAME COLUMN company_id TO merchant_id;
ALTER TABLE public.revenue_sharing RENAME COLUMN company_id TO merchant_id;
ALTER TABLE public.rider_expenses RENAME COLUMN company_id TO merchant_id;
ALTER TABLE public.tariff_notifications RENAME COLUMN company_id TO merchant_id;
ALTER TABLE public.wallets RENAME COLUMN company_id TO merchant_id;

-- 3. Migrate wallets.party_type data
UPDATE public.wallets SET party_type = 'merchant' WHERE party_type = 'company';

-- 4. Rewrite functions that hardcoded 'companies' / 'company_id' / 'company'
...
BEGIN
  IF NEW.settlement_approved = true AND (OLD.settlement_approved IS DISTINCT FROM true) THEN
    ...
    IF effective_rider_id IS NOT NULL AND rev.rider_percentage > 0 THEN
      ...
      WHERE party_type = 'rider' AND party_id = effective_rider_id AND merchant_id = NEW.merchant_id;

      IF w_id IS NULL THEN
        INSERT INTO wallets (party_type, party_id, user_id, merchant_id, balance)
        VALUES ('rider', effective_rider_id, rider_user, NEW.merchant_id, (net_income * rev.rider_percentage / 100))
    ...
    IF NEW.merchant_id IS NOT NULL AND rev.merchant_percentage > 0 THEN
      ...
      SELECT id INTO w_id FROM wallets
      WHERE party_type = 'merchant' AND party_id = NEW.merchant_id;
      ...
      IF w_id IS NULL THEN
        ...
        RETURNING id INTO w_id;
      ELSE
        UPDATE wallets SET balance = balance + (net_income * rev.merchant_percentage / 100), updated_at = now()
        WHERE id = w_id;
      END IF;

      INSERT INTO wallet_transactions (wallet_id, type, amount, description, delivery_id)
      VALUES (w_id, 'credit', net_income * rev.merchant_percentage / 100,
        'Revenue from delivery ' || order_ref || ' (' || rev.merchant_percentage || '% of D' || net_income || ' after D' || total_expenses || ' expenses)', NEW.id);
    END IF;
...
-- Rename column on revenue_sharing too: company_percentage -> merchant_percentage
ALTER TABLE public.revenue_sharing RENAME COLUMN company_percentage TO merchant_percentage;
...
BEGIN
  IF NEW.status = 'delivered' AND (OLD.status IS DISTINCT FROM 'delivered') THEN
    ...
    SELECT name INTO comp_name FROM merchants WHERE id = NEW.merchant_id;

    INSERT INTO delivery_alerts (delivery_id, alert_type, message)
    VALUES (
      NEW.id,
      'delivery_completed',
      COALESCE(rider_name, 'A rider') || ' completed delivery ' || COALESCE(NEW.order_reference, LEFT(NEW.id::text, 8)) || ' from ' || COALESCE(comp_name, 'unknown merchant')
    );
  END IF;
  RETURN NEW;
END;
...
BEGIN
  ...
  INSERT INTO tariff_notifications (merchant_id, tariff_id, message, location_name, old_amount, new_amount)
  VALUES (NEW.merchant_id, NEW.id, msg, NEW.location_name, old_amt, NEW.tariff_amount);
...
BEGIN
  IF NEW.manager_user_id IS NOT NULL AND (OLD.manager_user_id IS DISTINCT FROM NEW.manager_user_id) THEN
    IF EXISTS (
      ...
      WHERE party_type = 'merchant' AND party_id = NEW.id;
    ELSE
      INSERT INTO wallets (party_type, party_id, user_id, merchant_id, balance)
      VALUES ('merchant', NEW.id, NEW.manager_user_id, NEW.id, 0);
    END IF;
  END IF;
  RETURN NEW;
END;
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE party_type = 'rider'
      ...
      AND merchant_id IS NOT DISTINCT FROM NEW.merchant_id
  ) THEN
    INSERT INTO wallets (party_type, party_id, user_id, merchant_id, balance)
    VALUES ('rider', NEW.rider_id, r_user_id, NEW.merchant_id, 0);
  END IF;
  RETURN NEW;
END;
$function$;
...
BEGIN
  IF NEW.role = 'rider' THEN
    ...
    IF r_id IS NOT NULL AND NOT EXISTS (
      SELECT 1 FROM wallets WHERE party_type = 'rider' AND party_id = r_id AND merchant_id IS NULL
    ) THEN
      INSERT INTO wallets (party_type, party_id, user_id, balance)
  ...
  IF NEW.role = 'company_manager' THEN
    FOR comp_id IN SELECT id FROM merchants WHERE manager_user_id = NEW.user_id
    LOOP
      IF NOT EXISTS (
        SELECT 1 FROM wallets WHERE party_type = 'merchant' AND party_id = comp_id
      ) THEN
        INSERT INTO wallets (party_type, party_id, user_id, merchant_id, balance)
        VALUES ('merchant', comp_id, NEW.user_id, comp_id, 0);
      END IF;
    END LOOP;
  END IF;
