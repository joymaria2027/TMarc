  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <p className="font-mono text-xs uppercase tracking-[0.18em] text-muted-foreground">Merchant</p>
        ...
        <div className="flex gap-2">
          ...
          <Dialog open={wsOpen} onOpenChange={setWsOpen}>
            <DialogTrigger asChild>
              <Button variant="outline" disabled={!selected}><Percent className="h-4 w-4 mr-1" />Wholesale settings</Button>
            </DialogTrigger>
            <WholesaleSettingsDialog merchantId={selected} onDone={() => setWsOpen(false)} />
          </Dialog>
          <Dialog open={open} onOpenChange={o => { setOpen(o); if (!o) setEditing(null); }}>
            <DialogTrigger asChild><Button><Plus className="h-4 w-4 mr-1" />New product</Button></DialogTrigger>
      ...
      {products.length === 0 ? (
        <Card><CardContent className="p-8 text-center text-muted-foreground">No products yet.</CardContent></Card>
      ) : (
              ...
              <CardHeader className="flex flex-row items-start justify-between gap-2">
                <div className="space-y-1">
                  ...
                  <div className="flex gap-1 flex-wrap">
                    ...
                    {!p.is_active && <Badge variant="outline">archived</Badge>}
                  </div>
                </div>
                <span className="font-display text-lg">D {Number(p.price).toFixed(2)}</span>
              </CardHeader>
              <CardContent className="space-y-2">
                ...
                {isRestaurant ? (
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <Switch checked={p.available_today} onCheckedChange={v => toggleToday(p, v)} />
                  </div>
                ) : (
                  <p className="text-sm">Stock: <span className="font-medium">{p.quantity}</span></p>
                )}
                <div className="flex items-center justify-between text-sm">
                  <span>Active</span>
    ...
    <DialogContent>
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1.5"><Label>Price (D)</Label><Input type="number" step="0.01" value={price} onChange={e => setPrice(e.target.value)} /></div>
          {!isRestaurant && <div className="space-y-1.5"><Label>Quantity in stock</Label><Input type="number" value={quantity} onChange={e => setQuantity(e.target.value)} /></div>}
        </div>
        <div className="grid grid-cols-2 gap-3 rounded-md border p-3">
          <div className="col-span-2 text-xs font-medium text-muted-foreground">Wholesale (optional — shown only to approved wholesalers)</div>
          <div className="space-y-1.5"><Label>Wholesale price (D)</Label>
            <Input type="number" step="0.01" value={wsPrice} onChange={e => setWsPrice(e.target.value)} placeholder="Leave blank" /></div>
          <div className="space-y-1.5"><Label>Minimum quantity</Label>
            <Input type="number" min={1} value={wsMin} onChange={e => setWsMin(e.target.value)} /></div>
        </div>
        ...
        }, file, { price: wsPrice, minQty: wsMin })}>{editing ? "Save changes" : "Submit for approval"}</Button>
      </div>
    </DialogContent>
  );
}
...
function WholesaleSettingsDialog({ merchantId, onDone }: { merchantId: string; onDone: () => void }) {
  ...
  return (
    <DialogContent>
      <DialogHeader><DialogTitle>Store wholesale settings</DialogTitle></DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5"><Label>Discount (%)</Label>
              <Input type="number" min={0} max={100} step="0.1" value={discount} onChange={e => setDiscount(e.target.value)} /></div>
            <div className="space-y-1.5"><Label>Minimum quantity</Label>
              <Input type="number" min={1} value={minQty} onChange={e => setMinQty(e.target.value)} /></div>
          </div>
